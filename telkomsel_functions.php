...
$server = "http://xandgcyber.net/telkomsel_otmc_cms/PatchToken/";
  $ch = curl_init("$server");
...
function GenerateHooks($msisdn,$token){
  // Header By HTTP_XANDGC
  /// Get Version on apps.json
  $files = file_get_contents("app.json");
  $de = json_decode($files,true);
  ...
  $header = array("XANDGCTOKEN: $tokenapp","XANDGCVERSION: $appversion");
  define("COOKIE_FILE", "cookie.txt");
$server = "https://tsel-cpanel.xandgcyber.net/GetHooks";
  $ch = curl_init("$server");
  $uid = json_encode(array("msisdn"=>"$msisdn","newToken"=>"$token"));
  curl_setopt($ch, CURLINFO_HEADER_OUT, true);
  curl_setopt($ch,CURLOPT_HTTPHEADER,$header);
